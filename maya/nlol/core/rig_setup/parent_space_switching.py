...
class ParentSpacing:
    ...
    def build_parent_spaces(self):
        ...
        errors = []
        if not self.parents and not self.base_parents:
            errors.append(f'{self.control}: Must have "base_parents" if no "parents" object/s.')
        if self.separate_transforms and not self.parents:
            errors.append(f'{self.control}: "separate_transforms" requires "parents".')
        # require "base_parents" for "use_point_constraint"
        if self.use_point_constraint and not self.base_parents:
            errors.append(
                f'{self.control}: "base_parents" value required for "use_point_constraint"',
            )
        ...
        # ---------- check skip values ----------
        # check that base_parents exists if skip values exist
        if (
            self.parents
    ...
    def setup_main_parents(self):
        ...
        if not self.separate_transforms:
            ...
            for i, prnt_obj in enumerate(self.parents):
                ...
                # only skip if skip_scale and base_parents
                if not (self.skip_scale and self.base_parents):  # skip for skip_scale
                ...
                if not (self.skip_scale and self.base_parents):
                    cmds.connectAttr(
                        f"{ps_condition_nd}.outColorR",
            ...
            # check that not all transforms are skipped and base_parents is being used if skipping
